...
引擎元素级用**函数表** `gs_ParticleSystemLoadHelpFunction[KG3D_PARSYS_FEID_COUNT]` 分派(枚举 `KG3D_PARSYS_FEID_*`);发射器块内再 `switch(pBlock->byLauncherType)` 按 `PARSYS_LAUNCHER_*` 分派到各 per‑type `ReadData`。

⚠️ **差异重点有两个,缺一不可**:
1. **分派层**:元素块函数表缺 FEID(§2.1)、发射器类型 switch 缺 case(§2.2)、模块 switch 缺 case(§2.3)——新增类型。
2. **版本分支层**(§2.5)——**遍布解析全程,是普遍的落后来源**:代码里有大量版本标识数字(顶层 `dwVersion`/`PARSYS_VERSION_*`、各 block 的版本号、`MATINS_VERSION` 等),**新版本号恒大于老版本号,引擎新版本会在 `>= N` / `== N` 分支里加新字段/新读取**。复刻没跟到新版本就错位/漏抽。这不只发射器类型一种,而是**材质块/Track 块/launcher 块等各处都有版本分支**,需系统比对。
...
解析全程有大量版本标识数字,**新版本号恒大于老版本号,引擎新版本在 `>= N` / `== N` 分支里加新字段/新读取**,复刻没跟就错位/漏抽。不只发射器类型一种,材质块/Track 块/launcher 块等各处都有:
- **顶层 `dwVersion`/`PARSYS_VERSION_*`**:`HeaderPss.h` 的 `PARSYS_VERSION_00/10/11/11Classic`(注意是 `MAKELONG(低位,高位)` 复合值,非简单递增);`Pss.cpp` 顶层与各 block 用 `dwVersion >= PARSYS_VERSION_11` / `== PARSYS_VERSION_00` 等分支。引擎新加 `PARSYS_VERSION_12` 之类并加 `>= 12` 分支时,复刻要跟。
- **材质块 `MATINS_VERSION`/`MATINS_VERSION_PACKET`**:`Pss.cpp` 材质块按 `pMtlHead->dwVersion == MATINS_VERSION_PACKET/MATINS_VERSION/OLD` 分派——材质实例版本,新版本加字段。
- **`PARSYS_VERSION_CURRENT_EDITOR_VERSON`**:引擎编辑器当前写到文件的版本(`= PARSYS_VERSION_11`),复刻 `HeaderPss.h` 同步。引擎升这个值 = 新版本格式落地,复刻要补对应分支。
- **比对法**:grep `PARSYS_VERSION|dwVersion >=|dwVersion ==|MATINS_VERSION|>= PARSYS_VERSION` 各取两侧,核版本分支上限/版本常量是否一致。引擎新版本号/新分支 → 复刻补版本常量 + `>= N` 分支 + 对应新字段读取,对齐引擎 `SaveToFile` 该版本写入的字节。
...
- ⚠️ **跑完保留 AudioLabel.db,不要删**。`AudioLabel_baseline.db`/`AudioLabel_current.db` 是技能输出文件,跑完留在 `x64\Release\logs\` 供后续查阅/复算,**禁止 rm 删除**(清理临时只清 ScanFileList 等纯中间产物,AudioLabel.db 不算中间产物)。下一轮跑会因 InitDB 覆盖同名,但那是工具行为,不代表当轮产物要手动删。
...
> **只有真正改了代码才写报告**(§9)。若五层已对齐、没改码(如纯健康基线检查),不写报告、只在对话里说明。
