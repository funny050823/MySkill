...
默认:
  --root  $JX3_HD_Client/represent/rl(env 必须设,未设则须显式传 --root)
  --out   <repo>/x64/Release/logs/ScanFileList.txt
  --ext   krl
  --subset  可选:传一个目录(只在该目录下收集)或一个清单文件(每行一个路径,原样去重)。
          传了 --subset 时,--root 被忽略。

krl 技能用法(深扫 represent/rl 下 .krl):
  python regen_scanlist.py --root "$JX3_HD_Client/represent/rl" --ext krl \
...
_DEFAULT_ROOT = os.path.join(_CLIENT, "represent", "rl") if _CLIENT else None
...
def main():
    ...
    ap.add_argument("--root", default=_DEFAULT_ROOT,
                    help="收集 *.ext 的根目录(默认 $JX3_HD_Client/represent/rl;env 未设须显式传 --root)")
    ap.add_argument("--out", default=os.path.join(REPO_ROOT, "x64", "Release", "logs", "ScanFileList.txt"),
                    help="输出清单文件路径")
    ap.add_argument("--ext", default="krl", help="扩展名(默认 krl)")
