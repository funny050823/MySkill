...
# -*- coding: utf-8 -*-
"""
diff_srscene.py — 纯差异工具:对比两次全量 SRScene 扫描,列出修改前后数据差异(失败集+依赖路径)。

用途: SRScene代码同步技能里,改复刻代码前跑一次全量得 baseline,改+编译后再跑一次得 current,
      用本脚本列出差异(哪些 .SRScene 的失败/依赖变了)。

重要: 本脚本只报"差异",不判断差异算回归还是改善——好坏由报告/Claude 人工裁定(同 kmsc/Ani/tani/krl 方案)。
      资源对错是 SRScene.cpp 解析时 OnErrorByGBK/OnReadResourceFileByGBK 报的职责,不是 diff 的职责。

数据来源(ScanResult.db,SRScene 无音频、无专门成功表,同 kmsc/Ani/krl):
  (a) 解析失败: Result 中 File 以 .srscene 结尾(大小写不敏感) 且 ErrLevel=7
  (b) 依赖路径: Result 中 File 以 .srscene 结尾 的 (SonFile,SonExtName,ErrLevel,ErrType) 集合,按 SRScene 分组

差异类别(中性,不判好坏):
  changed        : 两侧都解析成功,但失败/依赖集变了
  appeared       : current 新进(baseline 失败/未扫到)——如修复版本错位导致 SMTemplate 抽到
...
无音频对比(SRScene 没有音频标签,不跑 SearchAudioLabel,无 --audiolabel)。

用法:
  python diff_srscene.py <baseline ScanResult.db> <current ScanResult.db> [--knownbad FILE] [--json] [--quiet]
...
def read_srscene_result(db):
    """读 Result 表里所有 File 以 .srscene 结尾(大小写不敏感)的记录。
    返回:
      failed  : set(norm(File)) 中 ErrLevel=7 的
      depend  : { norm(File): set( (norm(SonFile), norm(SonExtName), ErrLevel, ErrType) ) }
      scanned : set(norm(File))  (File 在 FileList 里的 srscene,用于 sanity)
    """
    failed = set()
    depend = {}
    scanned = set()
    try:
        con = connect(db)
        # 解析失败 + 依赖(都在 Result,File 以 .srscene 结尾,大小写不敏感)
        cur = con.execute(
            "SELECT File, SonFile, SonExtName, ErrLevel, ErrType FROM Result "
            "WHERE lower(File) LIKE '%.srscene'")
        ...
        # sanity: FileList 里的 srscene
        try:
            cur = con.execute("SELECT File FROM FileList WHERE lower(File) LIKE '%.srscene'")
            for r in cur.fetchall():
...
def main():
    ap = argparse.ArgumentParser(description="对比两份 SRScene ScanResult.db 的纯差异(失败集+依赖,无音频)")
    ap.add_argument("baseline", help="改码前 baseline ScanResult.db")
    ap.add_argument("current", help="改码后 current ScanResult.db")
    ap.add_argument("--knownbad", default=None, help="已知坏 SRScene 清单(每行一个路径)")
    ...
    b_fail, b_dep, b_scan = read_srscene_result(args.baseline)
    c_fail, c_dep, c_scan = read_srscene_result(args.current)

    # 纯差异:只报差异,不判好坏(同 kmsc/Ani/tani/krl 方案)
    all_files = set(b_fail) | set(c_fail) | set(b_dep) | set(c_dep) | b_scan | c_scan

    changed = []      # 两侧都解析成功,但依赖集变了(中性)
    appeared = []      # current 新进(baseline 失败/未扫到)——如修复版本错位
    ...
    if not args.quiet:
        ...
        if changed:
            print("\n[差异-依赖集变化] 以下 SRScene 两侧都解析但依赖集变了(好坏人工裁定):", file=hf)
            for x in changed[:20]:
                print("  %s  (%s -> %s)" % (x["file"], x["from"], x["to"]), file=hf)
            if len(changed) > 20:
                print("  ... 另有 %d 条" % (len(changed) - 20), file=hf)
        if appeared:
            print("\n[差异-新进] 以下 SRScene baseline 失败/未抽到、current 抽到依赖(如修复版本错位):", file=hf)
            for x in appeared[:20]:
                print("  %s" % x["file"], file=hf)
            if len(appeared) > 20:
                print("  ... 另有 %d 条" % (len(appeared) - 20), file=hf)
        if disappeared:
            print("\n[差异-消失] 以下 SRScene baseline 有、current 不在了(需关注):", file=hf)
