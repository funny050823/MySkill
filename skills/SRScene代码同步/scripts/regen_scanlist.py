...
默认:
  --root  $JX3_HD_Client/data/source/maps(env 必须设,未设则须显式传 --root)
  --out   <repo>/x64/Release/logs/ScanFileList.txt
  --ext   srscene
  --subset  可选:传一个目录(只在该目录下收集)或一个清单文件(每行一个路径,原样去重)。
          传了 --subset 时,--root 被忽略。

SRScene 技能用法(深扫 data/source/maps 下 .SRScene;磁盘扩展名大写,--ext srscene 用 lower 匹配):
  python regen_scanlist.py --root "$JX3_HD_Client/data/source/maps" --ext srscene \
...
# 仓库根 = 本脚本所在 .claude/skills/<技能>/scripts 上溯 4 级
...
_DEFAULT_ROOT = os.path.join(_CLIENT, "data", "source", "maps") if _CLIENT else None
...
def main():
    ap = argparse.ArgumentParser(description="生成 GBK 扫描清单 ScanFileList.txt")
    ap.add_argument("--root", default=_DEFAULT_ROOT,
                    help="收集 *.ext 的根目录(默认 $JX3_HD_Client/data/source/maps;env 未设须显式传 --root)")
    ap.add_argument("--out", default=os.path.join(REPO_ROOT, "x64", "Release", "logs", "ScanFileList.txt"),
                    help="输出清单文件路径")
    ap.add_argument("--ext", default="srscene", help="扩展名(默认 srscene;大小写不敏感,匹配磁盘 .SRScene)")
