...
# -*- coding: utf-8 -*-
"""
gen_report_kmsc.py — 为 UpdateCodeKmsc.md 生成"代码修改前后对比测试报告"的数据部分。

按 CodeReviewKMSC.md §6 要求,报告必须包含:
...
用法:
  python gen_report_kmsc.py --baseline-scan <b ScanResult.db> --current-scan <c ScanResult.db>
...
# kmsc 技能关注的 ScanResult.db 表(**不关注 Pss 表**)。read_table 探测列兼容缺失。
SCAN_TABLES = {
...
# AudioLabel.db 表(kmsc 协议动画相关 5 表)
AUDIO_TABLES = {
...
def main():
    ap = argparse.ArgumentParser(description="生成 UpdateCodeKmsc.md 的逐表对比数据(不关注Pss表)")
    ...
    print()
    print("### ScanResult.db 逐表对比(kmsc 技能关注 FileList/Result;**不关注 Pss 表**)")
