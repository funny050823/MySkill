...
# -*- coding: utf-8 -*-
"""
diff_kmsc.py — 纯差异工具:对比两次全量 kmsc 扫描,列出修改前后数据差异(失败集+依赖路径+音频标签)。

用途: kmsc代码同步技能里,改复刻代码前跑一次全量得 baseline,改+编译后再跑一次得 current,
      用本脚本列出差异(哪些 kmsc 的失败/依赖/音频变了)。

重要: 本脚本只报"差异",不判断差异算回归还是改善——好坏由报告/Claude 人工裁定(同 Ani 方案 a)。
      资源对错是 Kmsc.cpp/KMovieObject.cpp 解析时 OnErrorByGBK/OnReadResourceFileByGBK 报的职责,不是 diff 的职责。

数据来源(ScanResult.db + 可选 AudioLabel.db,kmsc 无专门成功表):
  (a) 解析失败: Result 中 File 以 .kmsc 结尾 且 ErrLevel=7
  (b) 依赖路径: Result 中 File 以 .kmsc 结尾 的 (SonFile,SonExtName,ErrLevel,ErrType) 集合,按 kmsc 分组
  (c) 音频标签: 可选 --audiolabel,AudioLabel.db File 表 .kmsc 部分的 (File,EventName,AudioFile) 三元组

差异类别(中性,不判好坏):
  changed        : 两侧都解析成功,但失败/依赖/音频集变了
  appeared       : current 新进(baseline 失败/未扫到)——如修复漏抽
...
用法:
  python diff_kmsc.py <baseline ScanResult.db> <current ScanResult.db>
...
def read_kmsc_result(db):
    """读 Result 表里所有 File 以 .kmsc 结尾的记录。
    返回:
      failed  : set(norm(File)) 中 ErrLevel=7 的
      depend  : { norm(File): set( (norm(SonFile), norm(SonExtName), ErrLevel, ErrType) ) }
      scanned : set(norm(File))  (File 在 FileList 里的 kmsc,用于 sanity)
    """
    failed = set()
    depend = {}
    scanned = set()
    try:
        ...
        # sanity: FileList 里的 kmsc
        try:
            cur = con.execute("SELECT File FROM FileList WHERE lower(File) LIKE '%.kmsc'")
            for r in cur.fetchall():
...
def read_audiolabel(db):
    """读 AudioLabel.db 的 File(File,EventName,AudioFile) 表,只取 .kmsc。
    返回 set of (norm(File), EventName, AudioFile)。表缺失返回空 set。
    """
    out = set()
    try:
        con = connect(db)
        cur = con.execute("SELECT File, EventName, AudioFile FROM File WHERE lower(File) LIKE '%.kmsc'")
        for r in cur.fetchall():
            out.add((norm(r["File"]),
        ...
        if "no such table" in str(e):
            print("warn: AudioLabel.db 无 File 表(%s)" % db, file=sys.stderr)
        else:
...
def main():
    ap = argparse.ArgumentParser(description="对比两份 kmsc ScanResult.db 的纯差异(失败集+依赖+音频)")
    ...
    ap.add_argument("--audiolabel", nargs=2, metavar=("BASE_AUDIO", "CUR_AUDIO"), default=None,
                    help="可选: 两份 AudioLabel.db,对比 .kmsc 音频标签(File,EventName,AudioFile)前后变化")
    ap.add_argument("--knownbad", default=None, help="已知坏 kmsc 清单(每行一个路径)")
    ...
    b_fail, b_dep, b_scan = read_kmsc_result(args.baseline)
    c_fail, c_dep, c_scan = read_kmsc_result(args.current)
