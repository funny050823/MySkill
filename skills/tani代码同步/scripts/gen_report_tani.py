...
# -*- coding: utf-8 -*-
"""
gen_report_tani.py — 为 UpdateCodeTani.md 生成"代码修改前后对比测试报告"的数据部分。

按 CodeReviewTani.md §5/§6 要求,报告必须包含:
  1. Scan.log 最后一行是否有 "日志正常关闭"(没有=Jx3SvnHookCheckTool.exe 执行失败)
  2. ScanResult.db 逐表对比(FileList/Result 等)——相同/不同/原因。**不关注 Pss 表**
  3. AudioLabel.db 逐表对比(File/FilterKmsc/LogInfo/MovieKrlTxt/NewMovieInfo)——相同/不同/原因
...
用法:
  python gen_report_tani.py --baseline-scan <b ScanResult.db> --current-scan <c ScanResult.db>
...
# tani 技能关注的 ScanResult.db 表(**不关注 Pss 表**)。read_table 探测列兼容缺失。
SCAN_TABLES = {
...
# AudioLabel.db 表(File=音频标签[tani 取 .tani 部分];其余 4 表为 kmsc 协议动画相关,tani 改动一般不影响,
# 依 spec "其他检查点都完全一样"逐表对比到,仅 .tani 相关的 File/LogInfo 会有差异)
AUDIO_TABLES = {
...
def main():
    ap = argparse.ArgumentParser(description="生成 UpdateCodeTani.md 的逐表对比数据(不关注Pss表)")
    ...
    print()
    print("### ScanResult.db 逐表对比(tani 技能关注 FileList/Result;**不关注 Pss 表**)")
    ...
    print()
    if audio_cmp:
        print("### AudioLabel.db 逐表对比(File/FilterKmsc/LogInfo/MovieKrlTxt/NewMovieInfo)")
        print("| 表 | baseline行数 | current行数 | 相同 | baseline独有(丢失) | current独有(新增) | 不同合计 |")
        print("|---|---|---|---|---|---|---|")
        for t in audio_cmp:
            print("| `%s` | %s | %s | %d | %d | %d | %d |" %
