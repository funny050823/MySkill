...
# -*- coding: utf-8 -*-
"""
gen_report_state.py — 为 UpdateCodeState.md 生成"代码修改前后对比测试报告"的数据部分。

按 CodeReviewState.md §5/§6 要求,报告必须包含:
  1. Scan.log 最后一行是否有 "日志正常关闭"(没有=Jx3SvnHookCheckTool.exe 执行失败)
  2. ScanResult.db 逐表对比(FileList/Result 等)——相同/不同/原因。**不关注 Pss 表**

State 无音频(不跑 SearchAudioLabel,无 AudioLabel.db),故无音频逐表对比(同 Ani/krl/SRScene)。
...
用法:
  python gen_report_state.py --baseline-scan <b ScanResult.db> --current-scan <c ScanResult.db>
...
# State 技能关注的 ScanResult.db 表(**不关注 Pss 表**)。read_table 探测列兼容缺失。
SCAN_TABLES = {
...
def main():
    ap = argparse.ArgumentParser(description="生成 UpdateCodeState.md 的逐表对比数据(不关注Pss表,无AudioLabel)")
    ...
    print()
    print("### ScanResult.db 逐表对比(State 技能关注 FileList/Result;**不关注 Pss 表**;无 AudioLabel)")
    ...
    print("> (State 无音频标签,不跑 SearchAudioLabel,无 AudioLabel.db 逐表对比——同 Ani/krl/SRScene。)")
    return 0
