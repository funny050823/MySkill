...
- 命令(在仓库根,先跑过 §1 check_env):
  ...
  ```
  - 脚本先编 RUST 依赖(KESMBase/ClipLib),再 rebuild `FileParse.sln`。设 `BUILD_SKIP_RUST=1` 可跳过 RUST 前置。
  - ⚠️ **Ani 尤其依赖 ClipLib**:VERVION3(Rust clip)解析直接用 `clip::Clip`(来自 ClipLib),lib/dll 缺 → 链接 LNK1104 / 运行时 dll 加载失败。
