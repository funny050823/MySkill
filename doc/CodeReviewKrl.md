# krl 解析复刻代码技能实现详细过程文档

本技能和`Pss代码同步`技能大体相同，我这里列出异同点。你帮我写个完整的`krl代码同步`技能。
...
- 复刻侧 不同于`Pss代码同步`
  ...
  - 复刻函数
    ...
    - 涉及文件：D:\QCBase\trunk\SourceCode\Tool\HDTools\KResourceReader\src\krl\krl.cpp
    - 复刻函数：KRL::ReadFile

  - 引擎原函数：
    - 项目：d:\JX3\trunk\Sword3\Source\Common\SO3Represent\SO3Represent_2019.vcxproj
    - 引擎文件：D:\JX3\trunk\Sword3\Source\Common\SO3Represent\Src\base\RLFile\KGRLLoader.cpp
    - 原函数：KGRLLoader::LoadUnitFromFile
...
### 3.2 音频标签（必须捞，重点项） krl没有音频标签
### 3.3 特效数据（写 m_pssInfo / 成员计数器）（必须捞，重点项）krl没有音频标签
...
### 第二步 深度检索 pss 出清单 不同于`Pss代码同步`
  - 深度扫描 $JX3_HD_Client/represent\rl\ 路径下 .krl 文件，生成清单
